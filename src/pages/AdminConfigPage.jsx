...
function ItemFields({ item, onChange, onUpload }) {
    ...
    return (
      <div className="space-y-8">
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center"><h4 className="font-display text-lg">Proyectos ({projects.length})</h4><button type="button" onClick={() => updateField('projects', [...projects, {title:'Nuevo', description:'', tag:'Narración', photo:''}])} className="text-xs font-bold text-terracota underline">+ Agregar Card</button></div>
          {projects.map((p, i) => (
            <div key={i} className="p-4 bg-marfil rounded-xl border border-arena/30 grid grid-cols-1 md:grid-cols-4 gap-4 items-center">
              <div className="md:col-span-1"><img src={p.photo || '/fotos/hero.jpg'} className="w-full h-20 object-cover rounded shadow" /></div>
              <div className="md:col-span-3 space-y-2">
                <input value={p.title} onChange={e=>updateProject(i, {title: e.target.value})} className="w-full bg-white border border-arena/20 rounded p-1 text-xs font-bold" />
                <input value={p.description} onChange={e=>updateProject(i, {description: e.target.value})} className="w-full bg-white border border-arena/20 rounded p-1 text-[10px]" />
                <button type="button" onClick={() => updateField('projects', projects.filter((_, idx)=>idx!==i))} className="text-[10px] text-red-400 hover:text-red-600">Eliminar</button>
              </div>
            </div>
          ))}
        </div>
  ...
  if (id === 'podcast') {
    return (
      <div className="space-y-6">
        <Input label="Título" value={content.title} field="title" />
        <Input label="Descripción" value={content.description} field="description" multiline />
        <Input label="Embed Spotify (URL src del iframe)" value={content.spotify_url} field="spotify_url" />
      </div>
    )
  }
  ...
  if (id === 'libros') {
    return (
      <div className="space-y-6">
        <Input label="Título de la Sección Libros" value={content.title} field="title" />
        ...
        <div className="p-4 bg-dorado/5 border border-dorado/20 rounded-xl">
           <p className="text-xs text-dorado font-bold">Nota:</p>
           <p className="text-[10px] text-cafe-medio">Los libros individuales se editan en la sección "Audiolibros" del menú principal.</p>
        </div>
      </div>
    )
  }
  ...
  if (id === 'contact') {
    return (
      <div className="space-y-6">
        ...
        <Input label="Email para el formulario" value={content.email} field="email" />
      </div>
    )
  }
